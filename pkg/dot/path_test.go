...
func TestNewPackagePath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := dot.NewPackagePath(tt.path)
			if tt.wantErr {
...
func TestNewTargetPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := dot.NewTargetPath(tt.path)
			if tt.wantErr {
				assert.True(t, result.IsErr())
			} else {
				assert.True(t, result.IsOk())
...
func TestPathEquality(t *testing.T) {
	path1 := dot.NewPackagePath("/home/user/.dotfiles").Unwrap()
	path2 := dot.NewPackagePath("/home/user/.dotfiles").Unwrap()
	path3 := dot.NewPackagePath("/home/user/other").Unwrap()
